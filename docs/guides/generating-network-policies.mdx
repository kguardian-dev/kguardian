...
A CiliumNetworkPolicy is namespaced, and so are the endpoint selectors inside it. From Cilium's [Kubernetes integration docs](https://docs.cilium.io/en/stable/security/policy/kubernetes/#policies-only-apply-within-the-namespace): "the policy only applies to pods within that namespace. It's possible, however, to grant access to and from pods in other namespaces" — by naming the namespace as a label on the peer, as in the docs' own example:
...
metadata:
  labels:
    ...
    app.kubernetes.io/part-of: kguardian
  name: web-cilium-policy
  namespace: prod
spec:
  description: Cilium network policy for pod web generated by kguardian
  egress:
  - toEndpoints:
    - matchLabels:
        k8s:app: sonarr
