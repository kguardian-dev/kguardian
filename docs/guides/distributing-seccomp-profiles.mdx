...
writes to nodes is always `SCMP_ACT_LOG`. Enforcing on a node currently means
placing the `SCMP_ACT_ERRNO` document there yourself (or via your own pipeline);
a distributable enforcing variant is planned. A too-tight profile breaks the
workload on its **next restart**, not immediately — so promote deliberately.
</Warning>
