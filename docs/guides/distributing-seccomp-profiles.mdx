...
The distributed profile files use `defaultAction: SCMP_ACT_LOG` — disallowed
syscalls are logged to the kernel audit log but nothing is blocked. Run in this
mode first and confirm the audit log shows nothing your workload actually needs
being denied.

```bash
# inspect what the enforcing profile would contain
...
<Warning>
`?action=` only changes what the **API renders** — the file the distributor
writes to nodes is always `SCMP_ACT_LOG`. Actually enforcing on a node currently
means placing the `SCMP_ACT_ERRNO` document there yourself (or via your own
pipeline). A `seccomp.distributeAction` value to switch this cluster-wide is a
planned follow-up. A too-tight profile breaks the workload on its **next
restart**, not immediately — so promote deliberately.
</Warning>
