...
kguardian's assistant runs 19 tools against the Broker: pod traffic, syscalls, pod and service inventory, network-policy audit verdicts, compute contention, workload security profiles, the image inventory, and generation of least-privilege `NetworkPolicy` and seccomp profiles from that observed baseline. The web UI is one way to reach them.
