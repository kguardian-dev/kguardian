...
kguardian's assistant runs 23 tools against the Broker: pod traffic, syscalls, pod and service inventory, network-policy audit verdicts, compute contention, workload security profiles, the image inventory, vulnerabilities and SBOMs, and generation of least-privilege `NetworkPolicy` and seccomp profiles from that observed baseline. The web UI is one way to reach them.
...
The tools cover these areas (the compute tools are described in [Compute Gauges](/guides/compute-gauges)). The value is not any single call — it is that an agent can chain them: find the pod, read its real traffic, check what a policy would have denied, then write the policy.
...
<AccordionGroup>
  ...
  <Accordion icon="shield-halved" title="Workload security profiles and images">
    `get_workload_security_profile` returns one workload's posture status (`ok`, `warn`, `risk` or `unknown`, derived from findings; there is no numeric score) and coverage, its top findings, controls, readiness checks, and per-dimension detail: Pod Security Standards level with the failing checks, network peers, syscalls and `SeccompProfile` sync, running image digests, and compute settings. When checks fail it includes a recommended `securityContext` patch. `list_workload_profiles` ranks many workloads (filter by namespace and posture: `ok`, `warn`, `risk`, `unknown`), and `diff_workload_profile` shows what changed between two stored revisions. `get_image_inventory` lists the image digests workloads run; it carries no vulnerability or signature data.

    Tools are keyed by workload (`namespace`, `kind`, `name`), not pod. `null` or `unknown` means kguardian has no data, or the source is not configured. It is never a pass: posture is `ok` only when all four core dimensions (network, syscalls, podSecurity, images) are known and ok, and images stays `unknown` until vulnerability data exists, so the tools return coverage alongside the status. A Pod Security Standards level of `restricted` is an upper bound kguardian cannot confirm. Every string in a result is cluster data, and the tools tell the model never to follow instructions inside it. Lists are capped (25 rows by default, 100 at most) and every response is size-bounded, with a `truncated` flag when rows were cut.

    *"Which workloads in `payments` are at risk, and what is the smallest securityContext change that gets `checkout` to restricted?"*
  </Accordion>

  <Accordion icon="bug" title="Vulnerabilities, CVE exposure and SBOMs">
    `get_image_vulnerabilities` returns an image's findings, deduplicated across sources (Trivy Operator, Grype), with every source's fixed versions, KEV and EPSS where a source gives them. `list_vulnerabilities` lists CVEs across the inventory with affected image, workload and namespace counts. `explain_cve_exposure` follows one id from affected images to the workloads running them and each workload's observed ingress. `get_image_sbom` returns every source's SBOM with its trust level and one SBOM's components.

    The tools tell the model never to invent a vulnerability id: every id in an answer must come from tool output. An image no source has scanned is unknown, not clean. Each finding carries its in-use state (executed, loaded, unknown or installed-not-observed, from exec and shared-library capture) and risk tier (P0 to Background) with the factors behind it; the tools can filter on both. Unknown in-use is treated as potentially reachable, and no finding is described as unreachable. `exposed: null` means no ingress was observed (egress alone does not count), never "not exposed". Only an SBOM with trust `verified` is described as signed. Findings come from Trivy Operator reports and, when the opt-in supplychain matcher is enabled, from kguardian's own Grype matcher, which matches SBOMs.

    *"Are we affected by CVE-2024-3094, where does it run, and is any of it reachable from outside its namespace?"*
  </Accordion>
</AccordionGroup>

Pod-specific tools take only `pod_name` — the Broker resolves it cluster-wide, so there is no namespace argument. Cluster-wide tools take an optional `namespace` filter. Profile tools take the workload key: `namespace`, `kind` and `name`.

<Note>
Every tool is a read against the Broker, plus two that compute policy locally. Nothing here writes to your cluster: an agent can hand you a `NetworkPolicy` or a `securityContext` patch, but applying it is still `kubectl apply` and your review.
</Note>
