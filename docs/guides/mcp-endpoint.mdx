...
<AccordionGroup>
  ...
  <Accordion icon="bug" title="Vulnerabilities, CVE exposure and SBOMs">
    ...
    The tools tell the model never to invent a vulnerability id: every id in an answer must come from tool output. An image no source has scanned is unknown, not clean. kguardian cannot yet tell which packages a workload loads, so no finding is described as unreachable. `exposed: null` means no ingress was observed (egress alone does not count), never "not exposed". Only an SBOM with trust `verified` is described as signed. kguardian reports what scanners found; it does not scan.
