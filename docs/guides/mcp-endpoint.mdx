...
<AccordionGroup>
  ...
  <Accordion icon="shield-halved" title="Workload security profiles and images">
    `get_workload_security_profile` returns one workload's posture score and coverage, its top findings, controls, readiness checks, and per-dimension detail: Pod Security Standards level with the failing checks, network peers, syscalls and `SeccompProfile` sync, running image digests, and compute settings. When checks fail it includes a recommended `securityContext` patch. `list_workload_profiles` ranks many workloads (filter by namespace and posture: `ok`, `warn`, `risk`, `unknown`), and `diff_workload_profile` shows what changed between two stored revisions. `get_image_inventory` lists the image digests workloads run; it carries no vulnerability or signature data.

    Tools are keyed by workload (`namespace`, `kind`, `name`), not pod. `null` or `unknown` means kguardian has no data, or the source is not configured. It is never a pass, and the score only covers known dimensions, so the tools return coverage alongside it. Lists are capped (25 rows by default, 100 at most) and every response is size-bounded, with a `truncated` flag when rows were cut.
