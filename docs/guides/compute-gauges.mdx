...
<Steps>
  ...
  <Step title="Sparklines">
    Expand the pod and the bar becomes two sparklines — the last 60 minutes,
    seeded from the broker's stored history when you open the card and kept
    current by the 5-second poll — with the current value and denominator.
    A stretch the pod did not report shows as a break in the line. Two chips can appear below them:
  ...
  <Step title="Risks">
    The Risks view (formerly Findings) lists the five compute kinds with the same rows as the
