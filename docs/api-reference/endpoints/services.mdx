...
object — selector + ports are the parts the advisor's and the
llm-bridge's NetworkPolicy generators actually read. The chart's
broker stores the full spec so future tooling can query other
fields without a controller round-trip.
...
llm-bridge's `list_services` tool.
