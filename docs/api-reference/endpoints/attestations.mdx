...
| `unknown` | Could not be checked (`reason`: `registry_auth` for a private image, `rate_limited`, `network`, `timeout`, `no_repo_digest`, `trust_root_unavailable`, ...). Not the same as unsigned. |
...
- `422` for an unknown `schema_version`, `verdict` or `signer_kind`, a body
  digest that differs from the path, or `checked_at` more than 60 s ahead.
