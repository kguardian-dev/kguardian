...
- **SBOMs are kept per source and never replace each other.** A `registry`
  SBOM is one found attached to the image in its registry. It carries
  `sbomTrust` (`unverified`, `attached-unbound` or `verified`; a registry
  SBOM that says nothing is stored as `unverified`) and the `attestation`
  it was found through. `verified: false` means no signature was checked:
  never show it as signed. `GET /images/{digest}/sbom` lists every
...
Keep the token in the supplychain component only.
...
2. Take their rows inside the window, matched on pod name **and**
   namespace. `flowsObserved` counts them (either direction).
...
4. `exposed` is `true` when any of those is non-zero, and `exposedVia`
   ...
   It is `false` when the pods had flows in the window and none of them
   was such ingress. It is `null` (unknown) when the broker knows no pods
   for the workload, or they had no flows in the window: nothing observed
