...
Grype matcher is enabled, matches SBOMs (Trivy's, plus any registry-attached
SBOM) itself; it posts one payload per image digest and source to the
broker. The broker stores them keyed by digest (never per
