...
The supplychain component reads Trivy Operator reports and, when its opt-in
Grype matcher is enabled, matches SBOMs itself: Trivy's, plus any
registry-attached SBOM when the opt-in `supplychain.sources.registry` source
is on. It posts one payload per image digest and source to the broker. The
broker stores them keyed by digest (never per pod), joins them to the
[image inventory](/architecture) and serves them to the UI, the assistant
and the CLI.

Findings come from Trivy Operator reports and, when the matcher is enabled,
from kguardian's own Grype matcher. kguardian never blocks a workload.
...
A CVE row's `tier` is `null` until the retention pass after an upgrade has
computed it ("not computed yet", never a low tier); a `tier` filter does not
match those rows. `tierFactors` lists what produced a finding's tier, for example
...
- **Findings are deduplicated across sources** on (vulnerability id,
  ...
  Severity and score are the highest any source gives, and a fix from any
  source makes it fixable.
- **KEV and EPSS are per CVE**, not per finding: `kev`, `kevDateAdded`,
  `epss` and `epssPercentile` on every finding (and on CVE rows) are
  resolved over every source's rows for that vulnerability id. `kev` is
  true if any source says true, false if one says false and none true,
  and `null` only when no source reports it; EPSS is the highest reported.
  So a Trivy finding (Trivy never reports KEV) shows `kev: true` when Grype
  lists the same CVE in KEV, even if the two name the package version
  differently, and its tier and `tierFactors` use that. The broker keeps these
  per-CVE facts in one row per CVE, updated at ingest and rebuilt each
  retention pass (so a lower EPSS takes effect, and CVEs whose findings
  were removed drop out, within one interval).
...
| `platform_manifest` | The imageID digest is one of the payload's `platform_manifests` (index to manifest, from the anonymous registry lookup, which is off unless `supplychain.registryLookup.enabled` is set; while off, `digestKind` is `unknown` and this rule never matches). | Exact |
