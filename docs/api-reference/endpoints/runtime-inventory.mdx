...
map entry per (container, capability, verdict); the first check of each
also sends an event naming the container. Heartbeats carry `cap_probe`, so
the broker knows which windows had the probe on.

Not counted: `CAP_OPT_NOAUDIT` checks (the kernel asking whether a task
would be privileged without the task needing it) and runc's own container
setup (tasks named `runc:[`).
...
It is `kg_runtime_coverage` plus the capability probe on every instance
in the window; its extra reason is `capabilities_not_tracked`. A
capability that was used is never recommended for dropping.
...
reason, the recommendation (`drop: ["ALL"]`, `add`: the used set) when the
evidence is sufficient, and `unusedAdded`: capabilities the container
adds today that it never used.
...
[
  {
    ...
    "workload_kind": "Deployment", "workload_name": "web",
    "container_name": "app", "image_digest": "sha256:0123…cdef",
    "capability": "NET_BIND_SERVICE", "granted": true, "count": 3,
