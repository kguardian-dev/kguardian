...
- **probed**: a `CAP_OPT_NOAUDIT` check that succeeded. Mostly the kernel
  ...
  One exception: a probed-only `SYS_ADMIN` is left out (`probedOmitted`)
  unless the container is privileged, because the recommendation keeps or
  sets `allowPrivilegeEscalation: false`, which gives `no_new_privs` and
  removes the seccomp case; the memory reserve is a heuristic, not
  functionality.
