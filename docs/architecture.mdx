...
<AccordionGroup>
  ...
  <Accordion icon="box-archive" title="Data Retention">
    ...
    - The image inventory (`images`, `workload_containers`) is sized by what runs: running pods refresh their rows continuously, and a digest no running pod has reported for `broker.imageInventory.retention.days` (default 30) is pruned, followed by images nothing references, with the same `intervalSeconds` and `batchSize` knobs.
