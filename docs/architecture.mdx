...
kguardian is designed as a distributed system: four core components (Controller, Broker, CLI, UI) plus an audit-mode evaluator (deployed by default) and an optional AI assistant (llm-bridge, running tools and policy/seccomp generation in-process) work together to observe, analyze, and secure your Kubernetes workloads.
...
<CardGroup cols={2}>
  ...
  <Card title="AI Assistant (Optional)" icon="robot" iconType="duotone">
    **Deployment:** llm-bridge (opt-in via chart values)
...
# Deployment: LLM bridge (when AI assistant is enabled)
