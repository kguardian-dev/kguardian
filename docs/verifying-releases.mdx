...
`ImageValidatingPolicy`, Sigstore policy-controller `ClusterImagePolicy`) if
you want the cluster to enforce them. With signature discovery on,
`kubectl kguardian images admission-policy` generates them from what it
verified on your running images.
