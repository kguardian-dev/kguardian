...
- [**AI Assistant**](/installation#ai-assistant) — optional LLM Bridge for natural-language queries, and an [MCP endpoint](/guides/mcp-endpoint) for your own client.
...
kguardian is built around four core components, plus an audit-mode evaluator (deployed by default) and an optional AI assistant (the LLM Bridge):
...
<Steps>
  ...
  <Step title="AI Assistant (Optional)">
    The optional **AI assistant** (the LLM Bridge) answers natural-language questions about cluster traffic and generates policies conversationally. The same tools can be served to an external MCP client — see [Connect an MCP Client](/guides/mcp-endpoint).
  </Step>
</Steps>
