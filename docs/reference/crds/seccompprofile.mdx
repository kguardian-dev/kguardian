...
| `denials` | Present whenever the Broker has an answer for `workloadRef`'s workload, **including when that answer is zero**. `observed` is the total across every syscall and action — it is the `DENIALS` printer column; `syscalls` is the distinct names, sorted; `lastSeen` is the most recent denial, RFC 3339. `observed: 0` means checked and clean; the block being **absent** means not known. |
