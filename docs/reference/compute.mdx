...
it collected. Two windows are fixed and run whatever `retentionDays` is: a
container's live row goes 10 minutes after its node last reported it, and a
node's row goes about an hour after the node last reported at all (the pass
runs every 600 s), so a node that left the cluster drops out of
`GET /compute/nodes` about an hour after its last report.
...
the `GET`s need a token with the `read` scope and the `POST`s need
`ingest` (see [Authentication](/api-reference/introduction#authentication)).
...
| `GET /compute/nodes` | `{ "nodes": [...] }` — every node's latest sample: pressure, context-switch rate, capacity, BPF map occupancy, and the `compute_enabled` / `compute_supported` / `contention_loaded` flags. A node that has not reported for about an hour is pruned from the list; its pods then read `pending`. |
