...
This gates the **UI route only**. It does not authenticate the Broker API, which the browser calls directly — see `broker.auth.enabled` for that, and note the frontend cannot hold a static token. It also does not cover the MCP endpoint above, which carries its own bearer token.
