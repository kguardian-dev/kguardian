...
therefore startup-complete. The set can include a few syscalls of the
container runtime's own setup (`runc init`); they widen the allowlist
slightly and never deny anything.
...
trigger at least one container restart under
`SCMP_ACT_LOG` before enforcing a profile recorded from fresh pods. Static
pods (whose mirror pod UID differs from the cgroup's) are not covered.
