...
- **hostNetwork pods** are not startup-complete. They share the node's network
  namespace, so once any hostNetwork pod on the node is registered, a starting
  hostNetwork container is captured by the registered path and credited to
  whichever hostNetwork pod holds the node's namespace entry. Per-container
  attribution by cgroup fixes this and is planned. Until then, use the
  restart-under-`SCMP_ACT_LOG` step above for hostNetwork workloads.
