...
`started_at IS NULL OR started_at <= at` and ordered
`is_dead ASC, started_at DESC NULLS LAST, time_stamp DESC`; the first
wins. Applied to the example above, the 2026-07-23 row cannot resolve to
`autobrr` (started 2026-08-04); it resolves to the CronJob pod that held
the IP then if its row still exists, and to nothing otherwise. A pod
with an unknown start is not excluded — there is nothing to compare —
but ranks after every candidate whose start is known.
...
in). A dead pod that was never re-posted stays unguarded and is ranked
