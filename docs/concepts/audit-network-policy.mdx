...
- It does not currently match `ipBlock` peers (would-be-denied flows from outside-the-cluster sources are not flagged in MVP).
- It does not currently resolve named ports (e.g. `port: http`); use numeric ports in audit policies for now. The matcher errs on "no match" rather than guessing.

## Limits + caveats

The matcher implements podSelector + namespaceSelector + numeric port + endPort range matching. Edge cases that warrant care:
