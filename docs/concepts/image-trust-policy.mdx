...
`STATE` says whether the counts mean anything: `evaluated`; `never-read`
(the broker has never been read, so the zeros are not "nothing to deny");
`broker-unavailable` or `broker-unauthorized` (every container is
`Unknown`). The `BrokerRead` condition carries the same with a reason
