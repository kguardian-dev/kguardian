...
NAME             CONTAINERS   WOULD-DENY   UNKNOWN   AGE
release-signed   12           2            1         3d
```
...
verdicts last changed. Status is written with server-side apply, so a
finding or error that no longer holds is removed, not left behind.
