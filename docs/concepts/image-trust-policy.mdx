...
expression or key). Status is written only when it changes. The evaluator
also serves the per-container results at `GET /image-trust` on its own
port (`?namespace=`, `?verdict=`).
