...
// Track ICMP echo requests (ping)
SEC("fentry/icmp_send")
int BPF_PROG(trace_icmp_send, struct sk_buff *skb_in, int type, int code, __be32 info)
{
    if (!skb_in)
        return 0;
    ...
    // Track ICMP sends (useful for correlation with drops)
    // The actual drop detection happens in netfilter hook
