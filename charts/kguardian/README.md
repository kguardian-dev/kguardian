...
![Version: 1.26.0](https://img.shields.io/badge/Version-1.26.0-informational?style=flat-square)
...
| broker.auth | object | `{"enabled":false,"existingSecret":"","keys":{"admin":"admin","ingest":"ingest","read":"read","supplychain":"supplychain"},"mode":"scoped","secretKey":"token"}` | Scoped bearer-token auth on the broker HTTP API. Without it any pod that can reach the broker Service can read everything and forge rows (pod specs, syscalls, mark_dead, seccomp status). With it, every route requires a token carrying the right scope:   read        → frontend (/api proxy, server-side), llm-bridge, CLI   ingest      → controller (also grants read)   supplychain → supply-chain component (also grants read)   admin       → optional operator token, grants everything The frontend's token never reaches the browser: the frontend server proxies /api to the broker and adds the header itself (GET/HEAD and the seccomp export POST only). That also means network reach to the frontend Service equals broker read access: put SSO in front of it (frontend.sso) and/or restrict it with a NetworkPolicy. /metrics stays open and carries workload namespace/name labels on the seccomp denial series; limit it with networkPolicy.allowMetricsFrom.  /health and /metrics stay open (kubelet probes + Prometheus can't send the token). Provide the tokens yourself in ONE Secret, one key per scope (not generated by the chart, so they're stable across upgrades):   kubectl -n <ns> create secret generic kguardian-broker-auth \     --from-literal=read="$(openssl rand -hex 32)" \     --from-literal=ingest="$(openssl rand -hex 32)" add --from-literal=supplychain=... / admin=... when you need them. Default false for now; supplychain.enabled=true refuses to render without it. |
| broker.auth.existingSecret | string | `""` | Name of an existing Secret holding the tokens. REQUIRED when enabled=true. |
| broker.auth.keys | object | `{"admin":"admin","ingest":"ingest","read":"read","supplychain":"supplychain"}` | Secret keys per scope (mode=scoped). read and ingest must exist; supplychain must exist when supplychain.enabled; admin is optional. |
| broker.auth.mode | string | `"scoped"` | "scoped" (a token per scope, from `keys`) or "shared" (the pre-scopes single token from `secretKey`, handed to every component and granting read + ingest; kept for upgrades, not recommended: the frontend then holds a token that can write). |
| broker.auth.secretKey | string | `"token"` | Key of the single shared token (mode=shared only). |
...
| broker.metrics.prometheusRule.enabled | bool | `false` | Create a PrometheusRule for prometheus-operator with alerts over the three `kguardian_seccomp_denial*` metrics above (fed by `seccomp.denials` — see that value for what feeds them). Off by default, like `serviceMonitor` above, for the same reason: it assumes a prometheus-operator Prometheus already exists and is configured to select PrometheusRule objects it did not create.  This toggle, `serviceMonitor.enabled`, and (if `broker.networkPolicy.enabled` is on) `broker.networkPolicy.allowMetricsFrom` are three independent switches. All three matter: a Prometheus that cannot reach the broker's /metrics — because nothing scrapes it, or because a default-deny NetworkPolicy blocks it — scrapes nothing, so enabling only this one renders alert rules that never fire because the series they name are simply absent, which alerts on absence (`absent()`) rather than the condition would surface, but plain threshold rules like these never will. |
| broker.metrics.prometheusRule.labels | object | `{}` | Extra labels on the PrometheusRule object (so prometheus-operator selects it — usually the same `release: kube-prometheus-stack` value set on `serviceMonitor.labels`; most installs require the matching label on both object kinds). |
...
| broker.metrics.serviceMonitor.labels | object | `{}` | Extra labels to add to the ServiceMonitor (so prometheus-operator picks it up — usually `release: kube-prometheus-stack`).  /metrics also carries `kguardian_seccomp_denials_total`, `kguardian_seccomp_denial_workloads` and `kguardian_seccomp_denial_rows_total` once `seccomp.denials.enabled` is capturing data — see `prometheusRule` below for alerts over them.  `kguardian_seccomp_denials_total` labels the denied workload's namespace `workload_namespace`, not `namespace`, so it survives the `namespace` target label prometheus-operator attaches to every ServiceMonitor job. Query and group on `workload_namespace`; there is deliberately no `honorLabels` setting here, because it would only cover operators scraping through this ServiceMonitor and a Prometheus with `overrideHonorLabels` pins it back to false anyway. |
...
| broker.runtimeInventory.retentionDays | int | `30` | Prune runtime inventory rows (the executables and shared libraries each workload container was seen running) not seen for this many days: a binary an old image ran, a deleted workload, a one-off exec. A path that keeps running is refreshed and never pruned. Set to 0 to disable pruning. The pass runs hourly and deletes `imageInventory.retention.batchSize` rows per statement. |
| broker.seccomp.denials.metricsIntervalSeconds | int | `15` | Seconds between refreshes of the cached `kguardian_seccomp_denial*` aggregate that `/metrics` reads (the broker deliberately never queries Postgres on the scrape path — see `broker.metrics.prometheusRule`). Deliberately its own timer, NOT `retention.intervalSeconds` above: `KguardianSeccompEnforcingProfileDenials` fires on a 5-minute `increase()` window, and reusing the retention loop's cadence (default 3600s) would leave that alert — the one covering an enforcing profile actively blocking or killing a workload right now — up to an hour stale. Resist merging these two timers even though they look redundant; that is exactly the simplification that would quietly break the critical alert's latency. Broker default is 15s if unset, with a 5s floor. Lower for faster alerting at the cost of more frequent aggregation passes; there is little reason to raise it above the default. |
| broker.seccomp.denials.retention | object | `{"batchSize":5000,"days":30,"intervalSeconds":3600}` | Retention for the `seccomp_denials` table — the kernel-verdict rows the broker ingests from `POST /seccomp/denials` (fed by the controller's `audit_seccomp` kprobe; see `seccomp.denials` above). Mirrors `broker.audit.retention` in shape and mechanism: a tokio task wakes every `intervalSeconds` and deletes rows whose `last_seen` is older than `days`, `batchSize` rows per DELETE. |
| broker.seccomp.denials.retention.batchSize | int | `5000` | Rows deleted per batched DELETE. Same [100, 100000] clamp as `broker.audit.retention.batchSize`, for the same reason: bounded lock hold and bounded WAL chunk per statement. |
| broker.seccomp.denials.retention.days | int | `30` | Retain seccomp_denials rows for this many days. Also bounds the window `kguardian_seccomp_denial_workloads` counts over and what `GET /seccomp/denials?since=` can return. Set to 0 to disable retention entirely (table grows unbounded). |
| broker.seccomp.denials.retention.intervalSeconds | int | `3600` | How often the cleanup pass runs, in seconds. Minimum 60. |
| broker.seccomp.profilesCacheTtlSeconds | int | `10` | Seconds the Broker serves the same rendered `GET /seccomp/profiles` body before rebuilding it from the database. It is the Broker's heaviest read (~3 MB and ~2.5s per call at ~2,000 workloads): every open UI tab polls it every 15s, and the Controller's seccomp distributor polled it every 30s per node until #1632 moved that to the per-workload route, which on a 44-node cluster meant ~90 rebuilds a minute and grew the Broker from 380 MiB to its 4 GiB limit until it OOM-looped. Within the TTL every caller gets the same bytes and concurrent misses wait for one rebuild. The cost is staleness: a SeccompProfile CR's deployment state or drift can lag this list by up to this many seconds (the per-workload `GET /seccomp/profiles/{ns}/{kind}/{name}` route is never cached). Broker default is 10 if unset. Set 0 to rebuild on every request. |
...
| broker.supplychain.maxDecompressedBytes | int | `8388608` | Largest supply-chain payload (vulnerabilities or SBOM page) the broker will inflate, in bytes. The supplychain component sends gzip bodies of at most 1 MiB; a body that inflates past this is refused with 413, and parsing also stops each list at its item cap, so one ingest peaks at about 62 MiB of heap at the default (one ingest runs at a time). Clamped to [1048576, 33554432]. Ingest also needs scoped broker auth: without a `supplychain` token configured the ingest routes answer 403. |
| broker.supplychain.retention | object | `{"attestationDays":7,"batchSize":50,"days":30,"intervalSeconds":300,"sbomPageTtlSeconds":3600,"unlinkedGraceHours":24}` | Retention for vulnerability and SBOM data. Rows are keyed by image digest, never by pod, and a payload is kept while an image it applies to runs. |
| broker.supplychain.retention.attestationDays | int | `7` | Image signature results (supplychain.signatureDiscovery) not re-checked for this many days are deleted: the component re-checks every running digest at least daily, so an older result means it stopped and the verdict is stale. 0 keeps them. Results for digests that left the image inventory are deleted either way. |
| broker.supplychain.retention.batchSize | int | `50` | Payloads deleted per transaction, clamped to [1, 1000]. |
| broker.supplychain.retention.days | int | `30` | Delete a payload once no inventory digest it applies to has had a workload container seen for this many days (and none is running now). 0 keeps payloads of images that stopped running. |
| broker.supplychain.retention.intervalSeconds | int | `300` | How often the pass runs, in seconds (minimum 60). Each pass also re-joins every payload to the inventory, so this is the longest a new digest waits before its scan shows up against it. |
| broker.supplychain.retention.sbomPageTtlSeconds | int | `3600` | A paged SBOM whose pages stop arriving is discarded this many seconds after its last page (minimum 60). |
| broker.supplychain.retention.unlinkedGraceHours | int | `24` | A payload no inventory digest matches (the scan arrived before the pod's first report, or the image never ran here) is kept this long after it was last received. |
| broker.supplychain.tiers | object | `{"epss":0.1,"inUseMinWindowHours":24,"unknownExposureAsExposed":true}` | Risk tiers for vulnerability findings (P0, P1, P2, Background). P0: package in use AND (CISA KEV or EPSS >= epss) AND exposed by observed ingress. P1: in use and critical/high, or the P0 factors without exposure. P2: in use and medium/low, or high with no fix and not exposed. Background: installed, never seen executed or loaded over a fully covered window. An unknown in-use state always tiers as in use. |
| broker.supplychain.tiers.epss | float | `0.1` | EPSS probability (0-1) at or above which a finding counts as likely exploited. |
| broker.supplychain.tiers.inUseMinWindowHours | int | `24` | Hours of continuous exec/library capture a container needs before a package in it can be called installed-but-not-observed (clamped to [1, 720]). |
| broker.supplychain.tiers.unknownExposureAsExposed | bool | `true` | Treat a workload with no captured ingress as exposed. With the default (true), a KEV finding (or one at or above the EPSS threshold) in a package in use lands in P0 when kguardian has observed no ingress for the workload at all: missing data pushes the tier up, never down. false treats no observed ingress as internal, so that finding is P1. |
...
| broker.traffic.retention | object | `{"batchSize":5000,"days":14,"intervalSeconds":3600,"maxRowsPerPod":0}` | Retention for the `pod_traffic` table, the observed flows every generated NetworkPolicy is built from. A tokio task in the broker wakes every `intervalSeconds` and deletes rows older than `days` whose pod no longer exists: dead, gone from the pod table, or an earlier pod that held the same name. A running pod's rows are kept however old: each flow is recorded once, when first seen, and a running pod does not report it again, so deleting them would drop rules from the next policy generated for that pod. The exception is a SUPERSEDED row: when a newer row of the same pod has the same direction, protocol, port, decision and in-cluster peer (same workload, or same pod/Service name when it has no owner), the older row is deleted once past `days`, because the newer one carries the same rule. That keeps a pod called by a CronJob from gaining a row per run. Peers with no stored identity (external, unresolved) are never superseded; see `maxRowsPerPod`. |
| broker.traffic.retention.batchSize | int | `5000` | Rows examined per batched DELETE (at most this many are deleted per statement). Same [100, 100000] clamp as `broker.audit.retention.batchSize`, for the same reason. |
| broker.traffic.retention.days | int | `14` | Keep traffic of departed pods for this many days. The default of 14 keeps two runs of a weekly CronJob; raise it for jobs that run less often than every `days` / 2 days, or their flows can be gone by the time you generate a policy. Set to 0 to disable retention entirely (table grows unbounded). |
| broker.traffic.retention.intervalSeconds | int | `3600` | How often the cleanup pass runs, in seconds. Minimum 60. |
| broker.traffic.retention.maxRowsPerPod | int | `0` | Opt-in cap on rows per pod. Off (0) by default. When a pod holds more rows than this, each pass deletes its oldest rows whose peer has no stored identity (external clients, scanners, unresolved IPs), regardless of age, until it is back at the cap, and the broker logs a warning naming the pod. Rows that name an in-cluster peer are never deleted by the cap, so a pod can stay over it. Useful for pods that see many external IPs, such as an ingress controller that preserves client IPs or a pod behind a public LoadBalancer. Finding over-cap pods reads the whole pod_name index once per pass. |
...
| controller.runtimeInventory.mode | string | `"off"` | Record which binaries each container executes and which files it maps executable (shared libraries), per container and image digest, for the "in use" signal on vulnerabilities and for drift (a binary run from the container's writable layer or from memory). `off` (default) loads nothing. `exec` loads the `sched_process_exec` probe only. `full` adds an fentry probe on `security_mmap_file` for libraries (falls back to exec-only on a kernel that cannot attach it). Both dedupe in the kernel: each (container, file) is reported once. Opt-in until its per-node overhead is measured. Containers already running when the probe attaches are backfilled once from `/proc`. Excluded namespaces and pods annotated `kguardian.dev/runtime-inventory: "off"` are not recorded. |
...
| evaluator | object | `{"affinity":{},"applicationSecurityProfiles":{"enabled":false,"installCRD":true,"resyncInterval":"5m","staleAfter":""},"autoscaling":{"enabled":false,"maxReplicas":5,"minReplicas":1,"targetCPUUtilizationPercentage":80},"container":{"port":8082},"enabled":true,"env":[],"image":{"pullPolicy":"IfNotPresent","repository":"ghcr.io/kguardian-dev/kguardian/evaluator","sha":"","tag":"v0.4.2"},"imagePullSecrets":[],"imageTrust":{"enabled":null,"interval":"5m"},"logLevel":"info","metrics":{"serviceMonitor":{"enabled":false,"interval":"30s","labels":{},"path":"/metrics","port":"http","scrapeTimeout":"10s"}},"nodeSelector":{"kubernetes.io/os":"linux"},"podAnnotations":{},"podDisruptionBudget":{"enabled":false,"maxUnavailable":"","minAvailable":1},"podSecurityContext":{"fsGroup":1000,"fsGroupChangePolicy":"OnRootMismatch","runAsGroup":1000,"runAsUser":1000,"seccompProfile":{"type":"RuntimeDefault"},"supplementalGroups":[1000]},"priorityClassName":"","replicaCount":1,"resources":{"limits":{"memory":"256Mi"},"requests":{"cpu":"50m","memory":"64Mi"}},"securityContext":{"allowPrivilegeEscalation":false,"capabilities":{"drop":["ALL"]},"privileged":false,"readOnlyRootFilesystem":true,"runAsNonRoot":true,"runAsUser":1000},"service":{"name":"kguardian-evaluator","port":8082,"type":"ClusterIP"},"serviceAccount":{"annotations":{},"automountServiceAccountToken":true,"create":true,"name":""},"startupProbe":{},"tolerations":[],"topologySpreadConstraints":[]}` | ----------------------------------------------------------------------- |
| evaluator.affinity | object | `{}` | Affinity rules for evaluator pod assignment |
| evaluator.applicationSecurityProfiles.enabled | bool | `false` | Report each workload's security profile (#1533) as an `ApplicationSecurityProfile` resource. You create one per workload (spec.workloadRef) and set spec.acceptedRevision to the profile revision you reviewed; the evaluator reads the profile from the broker and writes status (posture, per-dimension status, deviation from the accepted revision). Report-only: it never changes a workload and never creates or deletes these resources. Off by default. When broker.auth.enabled=true the evaluator presents the READ-scope token. Requires evaluator.enabled=true. |
| evaluator.applicationSecurityProfiles.installCRD | bool | `true` | Install the ApplicationSecurityProfile CRD as a release resource (kept on `helm uninstall`, like the SeccompProfile CRD). Set false if you manage CRDs yourself; then apply `charts/kguardian/files/kguardian.dev_applicationsecurityprofiles.yaml` before enabling. |
| evaluator.applicationSecurityProfiles.resyncInterval | string | `"5m"` | How often every profile is re-read from the broker (Go duration, floor 30s). Spec edits are picked up immediately. Profiles are read one at a time; the broker computes each one live. |
| evaluator.applicationSecurityProfiles.staleAfter | string | `""` | How long the last successfully read profile is still shown when the broker cannot be read (Go duration). Past it, posture and every dimension report `unknown`, with a reason naming the error and the last successful refresh. Empty = 3x resyncInterval; values below resyncInterval are raised to it. A broker that rejects the token (401/403) makes status unknown at once, regardless of this window. |
...
| evaluator.image.tag | string | `"v0.4.2"` | Evaluator version tag (auto-updated by release-please) |
| evaluator.imagePullSecrets | list | `[]` | List of image pull secrets for private registries |
| evaluator.imageTrust.enabled | bool | `nil` | Evaluate ImageTrustPolicy / ClusterImageTrustPolicy: compare every running container's image with the signers kguardian verified and write what would be denied to each policy's status. Report only; nothing is blocked. Unset (null) follows supplychain.signatureDiscovery.enabled. Reads the broker with the read token. |
| evaluator.imageTrust.interval | string | `"5m"` | How often every policy is re-evaluated (minimum 30s). |
...
| frontend.brokerAuth.allowSharedToken | bool | `false` | With broker.auth.mode=shared the single token also grants ingest, so by default the frontend gets NO token in shared mode and the UI gets 401s. Set true to hand it the shared token anyway (the /api proxy only forwards GET/HEAD and the seccomp export POST). Ignored in scoped mode, where the frontend always gets the read token. |
...
| seccomp | object | `{"denials":{"enabled":true,"intervalSeconds":10},"distribute":false,"distributeIntervalSeconds":"","installCRDs":true,"kubeletRoot":"/var/lib/kubelet"}` | Seccomp profile distribution, driven by the user-owned `SeccompProfile` CRD (`kguardian.dev/v1alpha1`). kguardian observes syscalls and renders a CR manifest you can commit; nothing reaches a node until you apply that CR. When `distribute` is on, the controller on every node watches `SeccompProfile` objects cluster-wide and writes each one to `<kubeletRoot>/seccomp/kguardian/<namespace>/<name>.json`, so a workload references it with `securityContext.seccompProfile.type: Localhost` + `localhostProfile: kguardian/<namespace>/<name>.json`. Deleting the CR deletes the file. kguardian never edits workloads and never writes a profile that is not backed by a CR. See docs/guides/distributing-seccomp-profiles. |
| seccomp.denials | object | `{"enabled":true,"intervalSeconds":10}` | Capture the kernel's own seccomp verdicts and turn them into a kguardian signal. A `SeccompProfile` with `defaultAction: SCMP_ACT_LOG` (kguardian's own default export — see above) makes the kernel write every would-be denial to the node's audit log (`type=SECCOMP` in `dmesg` / auditd); until this feature, kguardian never read that back, so the audit output of the very profile it generated was invisible to it. This attaches a kprobe on the kernel's `audit_seccomp()` — called whenever seccomp has decided an action is loggable — so the controller sees the verdict directly, attributes it to a pod/workload, and the broker stores it, exports `kguardian_seccomp_denial*` Prometheus metrics (see `broker.metrics.prometheusRule`), and mirrors it onto the `SeccompProfile`'s `status.denials` and `DenialsObserved` condition.  This answers a different question than the `Drift` condition. `Drift` is inferred from eBPF syscall observation and fires as soon as the workload makes a syscall your CR does not allow — even if no pod has ever referenced the profile. A denial only exists once a kernel filter built from that profile has actually been loaded and evaluated a syscall. Use `Drift` to catch a profile falling behind the workload; use denials to know the kernel is really acting on what you shipped. See docs/guides/distributing-seccomp-profiles. |
| seccomp.denials.enabled | bool | `true` | Load the `audit_seccomp` kprobe. Requires `CONFIG_AUDITSYSCALL=y` on the node kernel; where the symbol is absent the controller skips the probe and logs a warning rather than failing to start — the same degrade-gracefully pattern used for the syscall probe's IPv6 kprobe pair (see `controller/src/bpf.rs`, `kernel_can_fentry` / `symbol_location`). On by default: an audit-mode profile (`SCMP_ACT_LOG`) is inert without this — the kernel logs the verdict to the node's kernel ring buffer and nothing in kguardian ever reads it back.  Setting this also mounts the host's cgroup v2 root (`/sys/fs/cgroup`) read-only into the controller, the same mount `compute.enabled` asks for. A `hostNetwork: true` pod has no network namespace of its own, so its denials can only be attributed by the cgroup the calling task was in, and resolving a container's cgroup id needs that tree. Without it those verdicts are counted and dropped, and a workload whose denials nothing can attribute reports none at all. |
| seccomp.denials.intervalSeconds | int | `10` | Seconds between drains of the in-kernel denial counter map. Denials are counted in an LRU hash keyed on (netns, cgroup, generation, syscall, action) rather than streamed one ringbuffer event per denial — a compromised or malfunctioning process making the same blocked syscall in a tight loop must not be able to flood the controller — and the controller drains and clears that map on this interval, posting the counts to the broker. Matches the syscall probe's own drain cadence (`controller/src/syscall.rs`).  The Controller also declares this value to the Broker on every report, empty heartbeats included, and the Broker derives that node's staleness window from it: the node counts as still watching for `max(300, intervalSeconds * 3)` seconds after its last report, and a workload's `denials` block (and the `DenialsObserved` condition) falls back to "Unknown" only once no capturing node is inside its own window. Below ~100 the 300 s floor governs, which is the point: a node that misses a drain or two is late, not gone.  The supported range is 1-600 and the chart refuses anything larger. That staleness window is not only a timeout — it is how long kguardian will keep telling you a workload is clean on the strength of one old report — so the Broker caps it at 1800 s. Without the cap, a large interval meant that when the last capturing node went away (spot reclaim, autoscaler, a `CONFIG_AUDITSYSCALL=n` rebuild) every workload kept reading `total: 0` for hours with nothing watching. A node draining slower than 600 s is heard from less often than the Broker will vouch for it, so its workloads would sit at "Unknown" between reports: the honest answer, but not a useful setting.  Lower it for faster metric/status visibility at the cost of more frequent Broker POSTs; raise it on very large clusters to reduce Broker load — 600 is 60x fewer POSTs than the default. |
...
| supplychain.enabled | bool | `false` | Deploy the supply-chain component (#1533). It reads vulnerability and SBOM data about the images your workloads run and reports it; it never blocks or changes a workload. Off by default. Its default source is Trivy Operator (install Trivy Operator separately); registry SBOMs (sources.registry) and the Grype matcher (grype) are opt-in. Payloads are sent to the broker only when brokerIngest.enabled is true; otherwise they are logged. Requires broker.auth.enabled=true in scoped mode with a supplychain key; the chart refuses to render otherwise. |
...
| supplychain.grype.db.updateInterval | string | `"12h"` | How often to check for a newer DB (Grype also limits checks to once per 2h). Anchore publishes a new build daily. |
...
| supplychain.grype.enabled | bool | `false` | Match SBOMs against the Grype vulnerability database, as a second container in the supplychain pod (image supplychain-matcher). Off by default. It listens on 127.0.0.1 only and gets no Kubernetes or broker token. The input for each image is the UNION of Trivy's SbomReport and any registry SBOM; registry SBOMs are unverified and can only add packages, never remove Trivy's. No CPEs are sent, so a binary without a package URL is never matched. Egress: HTTPS to db.url's host (grype.anchore.io by default) for the DB listing and archive. The image exists from the first supplychain-matcher release (v0.1.0); until then leave this off or set image.tag to a pr-<N> build. |
...
| supplychain.grype.resources | object | `{"limits":{"memory":"512Mi"},"requests":{"cpu":"50m","memory":"128Mi"}}` | Matcher resources. Measured locally with grype v0.119.0: peak RSS about 78 MiB matching nginx:1.27 (151 components, 624 matches, about 2.4 s) and 330 MiB while downloading and unpacking the DB. SQLite page cache for the 3.1 GB DB is charged to the container but is reclaimable. With persistence disabled the chart also adds ephemeral-storage (request 7Gi, limit 9Gi) for the DB emptyDir. |
...
| supplychain.registryLookup.allowPrivateRegistries | bool | `false` | Allow lookups to RFC1918, CGNAT (100.64.0.0/10), ULA (fc00::/7) addresses, .local names and single-label (in-cluster) names, e.g. a homelab LAN registry. Loopback, link-local (including the cloud metadata address 169.254.169.254), unspecified and multicast addresses are always refused, checked after DNS resolution and on every redirect and token-realm request. |
| supplychain.registryLookup.enabled | bool | `false` | Ask each image's registry, anonymously, whether the reported digest is a multi-arch index or a single manifest, and list an index's platform manifests (digest_kind / platform_manifests in the payload). Never uses pull secrets or any credential: private images stay "unknown". Off by default and independent of brokerIngest.enabled; while off, every digest_kind is "unknown". Egress: HTTPS to the registries of your running images. |
| supplychain.resources | object | `{"limits":{"memory":"256Mi"},"requests":{"cpu":"10m","memory":"48Mi"}}` | Resource requests and limits. Memory grows with the number and size of Trivy reports in the cluster (the reports are cached); raise the limit on large clusters. With grype.enabled this container also holds up to 2000 images' SBOMs for matching; that footprint has not been measured yet, so watch its memory when you turn Grype on. |
...
| supplychain.signatureDiscovery.enabled | bool | `false` | Find out who signed every running image digest: cosign signatures (keyless and key) and Sigstore attestations (SLSA provenance, SBOMs), verified with sigstore-go. Reports only, never blocks a workload. Off by default: when on, the component contacts each image's registry anonymously (never with pull secrets, so a private image reads as "unknown", never "unsigned") and, unless trustedRoot is set, the Sigstore TUF CDN (tuf-repo-cdn.sigstore.dev). Needs brokerIngest.enabled=true. |
| supplychain.signatureDiscovery.interval | string | `"10m"` | How often every running digest is checked. Results are cached per digest for 24h (1h for "unknown"), so a pass mostly re-posts nothing. |
| supplychain.signatureDiscovery.publicKeys | object | `{}` | Public keys for images signed with a cosign key pair, as a map of key name to PEM public key (helm's set-file flag works well here). A signature one of these verifies is "verified" and attributed to the key; a key signature none of them verifies is "key_signed" (exists, not checked). Public keys only. |
| supplychain.signatureDiscovery.registryRps | int | `5` | Registry requests per second, all registries together. |
