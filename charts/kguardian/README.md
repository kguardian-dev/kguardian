...
| broker.supplychain.maxDecompressedBytes | int | `33554432` | Largest supply-chain payload (vulnerabilities or SBOM page) the broker will inflate, in bytes. The supplychain component sends gzip bodies of at most 1 MiB; a body that inflates past this is refused with 413 before it is parsed, so a decompression bomb costs at most this much memory (two ingests run at once). Clamped to [1048576, 134217728]. Ingest also needs scoped broker auth: without a `supplychain` token configured the ingest routes answer 403. |
