...
| ai.mcp.auth.allowUnauthenticated | bool | `false` | Serve /mcp with NO authentication. Every caller that can reach the Service gets unrestricted read access to your cluster's telemetry.  This exists as an explicit, reviewable opt-out rather than the chart silently allowing it, and rather than the chart forbidding it outright: if you enforce a default-deny NetworkPolicy or a mesh with mTLS in front of llm-bridge, the bearer token is genuinely redundant and refusing to render would be the chart overruling you about your own cluster. Setting it to true makes that judgement visible in your values file and in code review, which a missing token never is. If you are reaching /mcp over `kubectl port-forward` and nothing else in the cluster is hostile, this is defensible — but a token costs one `openssl rand` and removes the question. |
...
| ai.mcp.enabled | bool | `false` | Serve the MCP endpoint at /mcp. When false the path is not routed at all and 404s, so a disabled endpoint is indistinguishable from a build that never had one.  OFF by default because this hands cluster telemetry — pod traffic, syscalls, audit verdicts — to anything that can reach the Service, with no LLM in the path and no per-tool authorization. The Service is ClusterIP, which is NOT a boundary: every pod in the cluster can route to it unless a NetworkPolicy says otherwise, and this chart ships no llm-bridge NetworkPolicy. Treat enabling it as widening the blast radius of any compromised workload in the cluster, not just as opening a port for yourself. |
