...
| broker.supplychain.retention | object | `{"batchSize":50,"days":30,"intervalSeconds":300,"sbomPageTtlSeconds":3600,"unlinkedGraceHours":24}` | Retention for vulnerability and SBOM data. Rows are keyed by image digest, never by pod, and a payload is kept while an image it applies to runs. |
...
| controller.runtimeInventory.capabilities | bool | `false` | Also count the capability checks each container makes (fexit on `cap_capable`, or `security_capable` where that cannot be traced), per capability and verdict, deduplicated in the kernel. Feeds the workload profile's capabilities block and the evidence-based `capabilities` part of the securityContext recommendation. Requires `mode` exec or full. Off by default until its overhead is measured: capability checks are frequent on busy nodes. |
