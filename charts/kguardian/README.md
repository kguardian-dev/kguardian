...
| seccomp.denials.enabled | bool | `true` | Load the `audit_seccomp` kprobe. Requires `CONFIG_AUDIT=y` on the node kernel; where the symbol is absent the controller skips the probe and logs a warning rather than failing to start — the same degrade-gracefully pattern used for the syscall probe's IPv6 kprobe pair (see `controller/src/bpf.rs`, `kernel_can_fentry` / `symbol_location`). On by default: an audit-mode profile (`SCMP_ACT_LOG`) is inert without this — the kernel logs the verdict to the node's kernel ring buffer and nothing in kguardian ever reads it back. |
| seccomp.denials.intervalSeconds | int | `10` | Seconds between drains of the in-kernel denial counter map. Denials are counted in an LRU hash keyed on (netns, generation, syscall, action) rather than streamed one ringbuffer event per denial — a compromised or malfunctioning process making the same blocked syscall in a tight loop must not be able to flood the controller — and the controller drains and clears that map on this interval, posting the counts to the broker. Matches the syscall probe's own drain cadence (`controller/src/syscall.rs`). Lower it for faster metric/status visibility at the cost of more frequent broker POSTs; raise it on very large clusters to reduce broker load. |
