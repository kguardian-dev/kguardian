...
| broker.image.tag | string | `"1.8.0"` | Broker version tag (auto-updated by release-please) |
...
| evaluator.enabled | bool | `false` | Deploy the audit-mode policy evaluator. When false, the evaluator workload, RBAC, Service, and PDB/ServiceMonitor are skipped. The CRD itself ships in charts/kguardian/crds/ and is always installed by Helm regardless of this toggle.  Defaults to false because the evaluator image build pipeline lands in a follow-up PR; flip on once `ghcr.io/kguardian-dev/kguardian/evaluator` is publishing tags. |
