...
| broker.image.tag | string | `"1.15.0"` | Broker version tag (auto-updated by release-please) |
...
| controller.image.tag | string | `"1.12.0"` | Controller version tag (auto-updated by release-please) |
...
| frontend.image.tag | string | `"1.14.0"` | Frontend version tag (auto-updated by release-please) |
...
| llmBridge.image.tag | string | `"1.9.0"` | LLM Bridge version tag (auto-updated by release-please) |
...
| syscalls.captureLevel | string | `"full"` | Cluster-wide capture level: `full` (default, every syscall — the only tier that can publish a seccomp profile), `high`, `medium`, `low` or `custom`. Full capture is de-duplicated in BPF so it costs about the same CPU as `low`; pick a lower tier only if you never intend to publish seccomp profiles and want a smaller syscall table in the broker. The block above lists what each tier traces. |
