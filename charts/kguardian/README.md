...
| supplychain.registryLookup.enabled | bool | `nil` | Ask each image's registry, anonymously, whether the reported digest is a multi-arch index or a single manifest, and list an index's platform manifests (digest_kind / platform_manifests in the payload). Never uses pull secrets or any credential: private images stay "unknown". Unset (null) follows brokerIngest.enabled, so no lookups happen while ingest is off. Set false for air-gapped clusters. |
